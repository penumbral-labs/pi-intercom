...
test("stale asks classify only the exact opaque session ID", () => {
  ...
  assert.equal(stale.classify("ask-1", "session-a", 11), undefined);
  assert.equal(stale.classify("ask-1", "worker", 11), undefined);
});
